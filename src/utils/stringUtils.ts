...
export const shortenProductName = (name: string): string => {
  ...
  let cleaned = name.replace(/^([\s●◆■★]|【(?!新米|無洗米|玄米|特栽|特別栽培).*?】|（.*?）|[0-9０-９]+([.．][0-9０-９]+)?[KkＫｋ㎏GｇＧｇ]|[M]?[ﾎﾟﾘﾗﾐｿﾌｸﾗ]+|DHT?|RA|RZ|SFM?|PB|S|V|T|真空|ラミ|ポリ|別注|ＴＳ|TS|ＲＡ|ＲＺ|新版)+/g, (m) => {
    if (m.includes('新米') || m.includes('無洗米') || m.includes('玄米') || m.includes('特栽') || m.includes('特別栽培')) return m;
    return '';
  });

  // 2度洗いで確実に除去
  cleaned = cleaned.replace(/^([\s●◆■★]|【(?!新米|無洗米|玄米|特栽|特別栽培).*?】|（.*?）|[0-9０-９]+([.．][0-9０-９]+)?[KkＫｋ㎏GｇＧｇ]|[M]?[ﾎﾟﾘﾗﾐｿﾌｸﾗ]+|DHT?|RA|RZ|SFM?|PB|S|V|T|真空|ラミ|ポリ|別注|ＴＳ|TS|ＲＡ|ＲＺ|新版)+/g, '');
